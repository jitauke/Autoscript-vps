...
GITHUB_CMD="https://github.com/jitauke/Autoscript-vps/raw/"
NAMECOM=$(curl -sS https://raw.githubusercontent.com/jitauke/Autoscript-vps/main/ip | grep $MYIP | awk '{print $2}')
...
FIGHTERTUNNEL() {
    curl -sS https://raw.githubusercontent.com/jitauke/Autoscript-vps/main/ip >/root/tmp
...
ftTunneling() {
    ...
    IZIN=$(curl -sS https://raw.githubusercontent.com/jitauke/Autoscript-vps/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
